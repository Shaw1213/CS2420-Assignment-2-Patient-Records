...
public class Facility {
	...
	public ArrayList<CurrentPatient> lookupByPhysician(int physician) {
		var patientsForPhysician = new ArrayList<CurrentPatient>();
		for(var i = 0; i<patientList.size(); i++) {
			CurrentPatient patient = patientList.get(i);
			if(patient.getPhysician() == physician){
				patientsForPhysician.add(patient);
			}
		}
	...
	public ArrayList<CurrentPatient> getInactivePatients(GregorianCalendar date) {
		var patientsLastVisit = new ArrayList<CurrentPatient>();
		for(var i = 0; i<patientList.size(); i++) {
	...
	public ArrayList<Integer> getPhysicianList() {
		var physicianArray = new ArrayList<Integer>();
		for(var i = 0; i<patientList.size(); i++) {
			if(!physicianArray.contains(patientList.get(i).getPhysician())){
				physicianArray.add(patientList.get(i).getPhysician());
			}
		}
